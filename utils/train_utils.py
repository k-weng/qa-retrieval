...
def train(args, model, embedding, optimizer, criterion,
    ...
    for i, batch in enumerate(batches):
        ...
        hidden = forward_encoder(args, model, embedding,
...
def train_encoder_classifer(args, model_encoder, model_classifier, embedding,
    ...
    for i, batch in enumerate(batches):
        ...
        hidden = forward_encoder(args, model_encoder, embedding,
                                 encoder_title_ids, encoder_body_ids,
                                 padding_id)
        ...
        predictions = forward_classifier(args, model_classifier,
                                         embedding, classifier_title_ids,
                                         classifier_body_ids, padding_id)
...
def forward_classifier(args, model, embedding,
                       title_ids, body_ids, padding_id):
    ...
    predictions = (pred_t + pred_b) / 2

    return predictions


def forward_encoder(args, model, embedding, title_ids, body_ids, padding_id):
    embed_size = args.embed
